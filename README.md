...
- Reads only the file or directory you name; a directory scan is non-recursive and includes only immediate `*.jsonl` files. A final input component that is a symlink and symlink entries inside a selected directory are rejected rather than followed; ancestor path resolution follows operating-system path semantics.
